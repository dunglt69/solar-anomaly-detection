...
│   └── data/                   # Target folder for binary .npz datasets (git ignored)
...
1.  **Prepare Dataset**: Place the raw dataset `pv_fault_dataset.csv` inside `pv_fault_dataset-master/` and run:
    ...
    ```
    *   *Action*: Calculates power and ratio features on raw data, performs MinMax scaling, splits the dataset using a chronological day-based split (Days 1–12 train, Days 13–14 val, Days 15–16 test) to prevent temporal data leakage, and outputs binary numpy arrays (`.npz`) under `tools/data/`.
...
3.  **Export to ONNX**: Convert the PyTorch checkpoint to ONNX:
    ...
    ```
    *   *Action*: Compiles the checkpoint into `inception_fault_classifier.onnx` and outputs `model_metadata.json` (class names and model config) . Note: `scaler_params.json` (MinMaxScaler ranges for inference normalization) is generated by `prepare_dataset.py`, not this script.
